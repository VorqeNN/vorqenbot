...
module.exports = {
    ...
    async execute(interaction, client) {
        if(!interaction.member.permissions.has("MANAGE_CHANNELS")) return interaction.reply({ content: config.Locale.NoPermsMessage, ephemeral: true });

        let user = interaction.options.getUser("user");

        let alreadyBlacklistedLocale = config.Locale.notBlacklisted.replace(/{user}/g, `${user}`).replace(/{user-tag}/g, `${user.tag}`);
        ...
        let successfullyUnblacklistedLocale = config.Locale.successfullyUnblacklisted.replace(/{user}/g, `${user}`).replace(/{user-tag}/g, `${user.tag}`);
