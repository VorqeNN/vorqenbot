...
let utilityCount = 6
  ...
  helpembed.addField(`${config.HelpCommand.generalCategory.replace(/{cmdCount}/g, `${generalCount}`)}`, "``help``, ``ping``, ``suggest``, ``stats``")
  helpembed.addField(`${config.HelpCommand.ticketCategory.replace(/{cmdCount}/g, `${ticketCount}`)}`, "``add``, ``remove``, ``panel`` ``rename``, ``close``, ``pin``, ``delete``, ``alert``")
  helpembed.addField(`${config.HelpCommand.utilityCategory.replace(/{cmdCount}/g, `${utilityCount}`)}`, "``paypal``, ``crypto``, ``stripe``, ``calculate``, ``blacklist``, ``unblacklist``")
