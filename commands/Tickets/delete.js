...
exports.run = async (client, message) => {
    ...
    logEmbed.setTitle(config.Locale.ticketDeletedTitle)
    logEmbed.addField(`• ${config.Locale.logsDeletedBy}`, `> <@!${message.author.id}>\n> ${message.author.tag}`)
    logEmbed.addField(`• ${config.Locale.logsTicketAuthor}`, `> <@!${ticketAuthor.id}>\n> ${ticketAuthor.tag}`)
    logEmbed.addField(`• ${config.Locale.logsTicket}`, `> #${message.channel.name}\n> ${client.tickets.get(`${message.channel.id}`, "ticketType")}`)
